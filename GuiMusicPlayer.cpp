...
GuiMusicPlayer::GuiMusicPlayer()
{
	...
	wstring pPath = findBackgroundPicture();
	if (pPath.size())
	{
		haveBackgroundImage = true;										// 存在背景图片
		loadimage(&backgroundImage, pPath.c_str(), WIDTH, HEIGHT);	// 加载背景图片
		ld::ConsolePro().LogMessage(std::string("background image : ") + to_string(pPath));
	}
	else
	{
		haveBackgroundImage = false;
		ld::ConsolePro().LogWarning("background image not find");
	}
}

// 查找背景图片是否存在
// 查找到返回 true，否则返回 false
wstring GuiMusicPlayer::findBackgroundPicture()noexcept
{
	WIN32_FIND_DATA fileinfo;
	if (FindFirstFile(L"background.jpg", &fileinfo) != INVALID_HANDLE_VALUE)
		return L"background.jpg";
	else if (FindFirstFile(L"background.png", &fileinfo) != INVALID_HANDLE_VALUE)
		return L"background.png";
	else if (FindFirstFile(L"background.png", &fileinfo) != INVALID_HANDLE_VALUE)
		return L"background.png";
	else if (FindFirstFile(L"*.png", &fileinfo) != INVALID_HANDLE_VALUE)
		return fileinfo.cFileName;
	else if (FindFirstFile(L"*.jpg", &fileinfo) != INVALID_HANDLE_VALUE)
		return fileinfo.cFileName;
	else
		return L"";
}
...
bool GuiMusicPlayer::keyboardEnterEvent(int key)
{
	if (key == 0XD)
	{
		...
		if (currIndex != 0)													// 是否在播放列表中
		{
			...
			if (musicData.openMusic(currIndex) == false)									// 打开并播放 currIndex 指向的音乐
				keyboardEnterEvent(key);
		}
		else
		{
...
bool GuiMusicPlayer::keyboardUpDownEvent(int key)
{
	if (key == ((0XE0 << 8) | 0X48) || key == ((0XE0 << 8) | 0X50))				// 是否为上，下键
	{
		...
		if (currIndex == 0)														// 鼠标不在播放列表，进行上一曲，下一曲操作
		{
			...
			{
				if (musicData.openMusic(rand() % musicData.musicName.size()) == false)
					keyboardUpDownEvent(key);
			}
			else
			{
				if (dir == -1)													// 上一曲
